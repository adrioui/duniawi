...
Open a todolist (`todo_write`) with one item per step below.
...
## 3. Explore via delegation

Delegate codebase exploration (the **guard-the-context-window** principle skill).

- Fan explorers out through the `workflow` tool: one `agent()` per subsystem, returning file pointers, conventions, dependencies, test infrastructure, and entry points. No inlined dumps.
- Assign models per the model table: `configured code route` for code-oriented exploration, `configured judgment route` for judgment-heavy analysis. Use `configured workers route` for cheap parallel sweeps, falling back to `configured code route` when unavailable.
- For a single explorer, a plain `subagent` call is fine.
...
If a phase creates or edits a skill, the phase instructs the implementer to follow the **authoring-a-skill** playbook (Agent Skills format: kebab-case name + description frontmatter, one-level bundles).
...
**Runtime.** Exercise the feature on the real artifact via bash/scripts on the matching surface:

- CLIs and TUIs: drive the real binary in a scratch environment; capture before/after output.
- Web UIs: build, serve, and verify against the running artifact (curl the served output, screenshot via script if available).
- No scriptable path for the touched surface: flag it in the plan.
