...
4. **Spawn N parallel candidates** on different models using DSH **subagent** (background) or the **workflow** tool's `agent(prompt, {model, label, phase})` hook. Each works in its own sanitized dir; same prompt to each.
5. **Spawn one blinded judge** on a different model family using a separate **subagent** or **workflow** agent call. Judge sees outputs by sanitized label and the rubric, never a model name.
6. **Verify the chain from handoff docs, not self-report.** Read each candidate's handoff document (produced via the DSH **handoff** skill). Look at which files each candidate actually opened. Citing a principle is not reading its leaf skill, and reading it is not applying it. Grade chain-following from the files it really read plus the shape of the code, never from the candidate's own claims.
