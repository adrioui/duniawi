...
**You own the exit condition. Define done, then drive to it without stopping.** For "going to bed" / "run until done".
...
2. Create a DSH **goal** via `create_goal` with the exit condition as the objective. Automatic continuation rounds will drive the loop. Pick the wake mechanism: an event to watch (CI, a merge, a ref advancing) gets a watcher subagent that wakes you on the event, with a long time-based heartbeat as fallback. No event gets a fixed-interval heartbeat sized to when the result is worth re-checking.
