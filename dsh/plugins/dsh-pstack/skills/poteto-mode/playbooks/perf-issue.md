...
1. Capture a baseline trace on the matching surface via bash/scripts against the real artifact.
...
3. Plan the fix from the trace. If it crosses a function boundary, `architect` first. Delegate implementation to a subagent (code + delegate implementation → `configured code route`); review the diff. Capture a post-fix trace.
